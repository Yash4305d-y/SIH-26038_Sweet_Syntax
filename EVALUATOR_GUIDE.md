...
- **Domain-Shift Monitor**: The domain-shift monitor is an experimental distribution-monitoring component. Its thresholds were selected from the APTOS validation distribution and were not tuned on APTOS test, Messidor-2, or IDRiD. Current validation did not establish a consistent relationship between representation mismatch and individual prediction error. Therefore the monitor does not modify predictions or Referable Probability. APTOS is the current reference domain. External datasets are stress-test domains, not representative Indian deployment data. Representative Indian field data would be required to characterize deployment-domain shift.
