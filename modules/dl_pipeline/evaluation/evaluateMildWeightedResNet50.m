% src/evaluateMildWeightedResNet50.m
% Evaluates the mild square-root weighted ResNet-50 network on the test split.

function evaluateMildWeightedResNet50()
    % Setup paths
    ...
    netPath = fullfile(modelsDir, 'mild_weighted_resnet50_smoketest.mat');
    if ~exist(netPath, 'file')
        error('Trained network not found at %s. Please run trainMildWeightedResNet50.m first.', netPath);
    end
    
    % 1. Load trained mild weighted ResNet-50
    disp('Loading trained mild weighted network...');
    ...
    % Print Metrics
    disp('--- Final Mild Weighted Evaluation Metrics ---');
    ...
    % Compare with baseline and strong weighted models
    baselinePath = fullfile(resultsDir, 'baseline_metrics.mat');
    strongPath = fullfile(resultsDir, 'weighted_metrics.mat');
    
    if exist(baselinePath, 'file') && exist(strongPath, 'file')
        disp('================================================================');
        disp('                    THREE-WAY MODEL COMPARISON                  ');
        disp('================================================================');
        
        base = load(baselinePath);
        strong = load(strongPath);
        
        fprintf('%-15s | %-12s | %-15s | %-15s\n', 'Metric', 'Baseline', 'Strong Weighted', 'Mild Weighted');
        fprintf('----------------------------------------------------------------\n');
        fprintf('%-15s | %-12.4f | %-15.4f | %-15.4f\n', 'Accuracy', base.accuracy, strong.accuracy, accuracy);
        fprintf('%-15s | %-12.4f | %-15.4f | %-15.4f\n', 'Macro F1', base.macroF1, strong.macroF1, macroF1);
        fprintf('%-15s | %-12.4f | %-15.4f | %-15.4f\n', 'QWK', base.qwk, strong.qwk, qwk);
        fprintf('----------------------------------------------------------------\n');
        disp('Recall / Sensitivity Comparison:');
        for i = 1:N
            fprintf('Grade %s        | %-12.4f | %-15.4f | %-15.4f\n', ...
                char(classes(i)), base.recall(i), strong.recall(i), recall(i));
        end
        disp('================================================================');
        disp(' ');
    else
        disp('Warning: Baseline or Strong Weighted metrics files are missing. Cannot show 3-way comparison.');
    end
    
    % Save predictions to CSV
    csvOutPath = fullfile(resultsDir, 'mild_weighted_test_predictions.csv');
    ...
    % Save metrics
    metricsPath = fullfile(resultsDir, 'mild_weighted_metrics.mat');
    ...
    cm.Title = 'Mild Weighted ResNet-50 Test Confusion Matrix';
    cm.RowSummary = 'row-normalized';
    cm.ColumnSummary = 'column-normalized';
    
    cmPath = fullfile(resultsDir, 'mild_weighted_confusion_matrix.png');
    saveas(fig, cmPath);
    close(fig);
