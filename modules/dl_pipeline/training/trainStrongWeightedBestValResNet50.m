% src/trainStrongWeightedBestValResNet50.m
% Trains a pretrained ResNet-50 network using a moderately strong (Power 0.75) 
% class-weighted cross-entropy, tracking validation loss over 10 epochs, 
% and retaining the best-performing checkpoint on the validation set.

function trainStrongWeightedBestValResNet50()
    % Setup paths relative to the project root
    scriptPath = mfilename('fullpath');
    [srcDir, ~, ~] = fileparts(scriptPath);
    projectDir = fileparts(srcDir);
    
    % Use new output directories for this strong weighted run
    modelsDir = fullfile(projectDir, 'models', 'strong_weighted_resnet50_bestval');
    ...
    % Calculate Strong (Power 0.75) Inverse-Frequency Class Weights
    disp('Calculating strong class weights from training set ONLY...');
    countsTbl = countEachLabel(imdsTrain);
    N_i = countsTbl.Count;
    N = sum(N_i);
    K = length(classes);
    
    % Formula: rawWeight_i = (N / N_i)^0.75
    % This sits exactly between the mild sqrt(0.5) and extreme inverse (1.0) weighting
    rawWeights = (N ./ N_i).^0.75;
    ...
    disp(' ');
    disp('--- Calculated Strong (Power 0.75) Class Weights ---');
    for i = 1:K
    ...
    weightsPath = fullfile(resultsDir, 'strong_class_weights.mat');
