% src/evaluateStrongWeightedBestValResNet50.m
% Evaluates the moderately strong (Power 0.75) weighted ResNet-50 network (BEST Validation Checkpoint).

function evaluateStrongWeightedBestValResNet50()
    % Setup paths
    ...
    resultsDir = fullfile(projectDir, 'results', 'strong_weighted_bestval');
    if ~exist(resultsDir, 'dir'), mkdir(resultsDir); end
    
    % Load the BEST validation ResNet-50 network
    netPath = fullfile(modelsDir, 'strong_weighted_resnet50_bestval', 'bestval.mat');
    ...
    disp('Loading BEST trained strong weighted network...');
    ...
    [~, maxIdx] = max(scores, [], 2);
    predStrings = string(classes(maxIdx));
    YPred = categorical(predStrings, classes);
    ...
    % Print Metrics
    disp('================================================================');
    disp('       Strong Weighted Best Validation Model Evaluation           ');
    disp('================================================================');
    
    fprintf('Model evaluated: BEST validation checkpoint\n');
    ...
    csvOutPath = fullfile(resultsDir, 'strong_weighted_bestval_predictions.csv');
    
    [~, fileNames, ~] = fileparts(paths);
    
    idCodes = string(fileNames(:));
    trueLabels = string(YTest(:));
    predictedLabels = string(YPred(:));
    
    if ~(numel(idCodes) == numel(trueLabels) && numel(trueLabels) == numel(predictedLabels))
        error('Prediction output size mismatch: idCodes=%d, trueLabels=%d, predictedLabels=%d.', ...
            numel(idCodes), numel(trueLabels), numel(predictedLabels));
    end
    
    outTable = table(idCodes, trueLabels, predictedLabels, ...
        'VariableNames', {'id_code', 'true_diagnosis', 'predicted_diagnosis'});
    
    writetable(outTable, csvOutPath);
    
    % Save metrics
    metricsPath = fullfile(resultsDir, 'strong_weighted_bestval_metrics.mat');
    ...
    cm.Title = 'Strong Weighted Best Validation Model';
    cm.RowSummary = 'row-normalized';
    cm.ColumnSummary = 'column-normalized';
    
    cmPath = fullfile(resultsDir, 'strong_weighted_bestval_confusion_matrix.png');
    saveas(fig, cmPath);
    close(fig);
    
    % Save a TXT summary
    txtPath = fullfile(resultsDir, 'strong_weighted_bestval_summary.txt');
    fid = fopen(txtPath, 'w');
